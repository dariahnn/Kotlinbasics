...
class NightClub(override val minimumAge: Int,
    ...
    : ClubRules{
        ...
        private val bannedMembers = mutableListOf<String>()
    companion object {
        val timeFormatter: DateTimeFormatter = DateTimeFormatter.ofPattern("HH:mm:ss")
    }

    override fun canEnter(member: Member): Boolean {
        return member.age >= minimumAge && !bannedMembers.contains(member.id)
    }
    fun addMember(member: Member){
        // validating if the member can enter my club
        if(canEnter(member)){
            // add our member to our list of members
            currentMembers.add(member)
            println("${member.name} (${member.membership})entered at ${member.entryTime.format(timeFormatter)}")
    ...
    // this function will allow my objecs to be entered in the banned section listing
